...
$query = "select * from mensalidade where id_aluno = $id_aluno";
...
<section>
<div class="container ">
<div class="row  mt-5 fonte-20"><p>Mensalidades</p></div>
    ...
    <tbody>
        <?php 
            while ($linha =  mysqli_fetch_array($consulta_pagamentos)) {
                ...
                echo '<td>'.$linha['juros'].'</td>';
                echo '<td>'.$linha['data_vencimento'].'</td>';
                echo '<td>'.$linha['observacao'].'</td>';
                $pagamento = intval($linha['status']);
                if ($pagamento == 0){ 
               ?>
                                <td><a href="pagar-mensalidade.php?id_pagamento=<?php echo $linha['id'];?>">
                ...
                echo '<td><?xml version="1.0" encoding="iso-8859-1"?>
                <!-- Generator: Adobe Illustrator 19.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0)  -->
                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;height:40px;" xml:space="preserve">
